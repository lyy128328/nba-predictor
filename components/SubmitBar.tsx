...
export function SubmitBar({ totalSeries, completedSeries, editableSeries, saving, onSubmit }: Props) {
  const complete = totalSeries === 0 ? 0 : Math.round((completedSeries / totalSeries) * 100);
  const disabled = saving || editableSeries === 0;
  ...
  return (
    <div className="sticky bottom-4 z-10 rounded-[28px] border border-slatewarm-950 bg-slatewarm-950 p-4 text-white shadow-card">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        >
          {saving ? "Saving..." : editableSeries === 0 ? "All series locked" : "Submit picks"}
        </button>
      </div>
    </div>
  );
